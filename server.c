...
int http_request_init(http_request *arr) {
  sarr_init(&arr->method, 3);
  sarr_init(&arr->path, 16);
  sarr_init(&arr->version, 8);
...
int http_request_free(http_request *arr) {
  sarr_free(&arr->method);
  sarr_free(&arr->path);
  sarr_free(&arr->version);
...
void http_request_printInfo(http_request *arr) {
  printf("HTTP Request:\n  Method: %s\n  Path: %s\n  Version: %s\n  Headers:\n",
         arr->method.ptr->ptr, arr->path.ptr->ptr, arr->version.ptr->ptr);
...
int parse_first_line(http_request *arr, char **curr_pos, size_t *length) {
  ...
  *end_line = '\0';
  ...
  addstr(&arr->method, method);
  addstr(&arr->path, target);
  addstr(&arr->version, version);
